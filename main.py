...
for i in range(num_customers):
    ...
    full_name = fake.name()
    # Split the full name into first and last name; cause in our ER diagram we have only "name" for customers.
    ...
    discount_eligibility = random.choice(['None', 'Silver', 'Gold']) if c_type == 'corporate' else 'None'
    
    cursor.execute("""
        INSERT INTO Customer
            (name, phone_number, email, customer_type, discount_eligibility, company_name, tax_number)
        VALUES (?, ?, ?, ?, ?, ?, ?);
    """, (full_name, phone_number, email, c_type, discount_eligibility, company, tax_nr))
...
for i in range(num_suppliers):
    ...
    cursor.execute("INSERT INTO Supplier (name, location) VALUES (?, ?);", (s_name, location))
...
for i in range(num_employees):
    ...
    # Force the first employee to be a repair tech, randomise the rest
    occupation = 'repair tech' if i == 0 else random.choice(occupations)
    cursor.execute("""
        INSERT INTO Employee (first_name, last_name, phone, email, occupation)
        VALUES (?, ?, ?, ?, ?);
    """, (emp_f, emp_l, phone, email, occupation))

# 4. Parts Catalog
num_parts = 30
part_categories = ['screen', 'battery', 'camera', 'motherboard', 'housing', 'port', 'fingerprint reader']
for i in range(num_parts):
    ...
    cursor.execute("""
        INSERT INTO Part (serial_number, quality, manufacturer, part_type, purchase_price, name)
        VALUES (?, ?, ?, ?, ?, ?);
    """, (serial_number, quality, manufacturer, part_type, cost, chosen_supplier))
...
cursor.execute("SELECT employee_id FROM Employee WHERE occupation = 'repair tech';")
...
phone_models = [
    ...
    ('Samsung', 'Galaxy S21'), ('Samsung', 'Galaxy S22'), ('Samsung', 'Galaxy A53'),
    ('Google',  'Pixel 7'), ('Xiaomi',  'Redmi Note 11')
]

repair_types = [
    ('Screen replacement',     120, 250), ('Battery replacement',     60, 120),
    ('Charging port repair',    70, 140), ('Back glass replacement',  80, 180),
    ('Water damage diagnosis',  25,  25), ('Data recovery',           40, 150)
]


statuses = ['Waiting', 'In progress', 'Waiting for parts', 'Done', 'Picked up', 'Unclaimed']

# 5. Tickets (~200), Repairs (~250), UsedParts (~400), and Payments
num_tickets = 200
...
for i in range(num_tickets):
    ...
    speed_estimate = random.choice(['1 Hour Rush', '1-2 Days Standard'])
    brand, model = random.choice(phone_models)
    device_name = f"{brand} {model}"
    password = fake.password(length=random.randint(4, 8)) if random.random() < 0.7 else None
    
    intake_date = fake.date_between(start_date=datetime(2026, 1, 1), end_date=datetime(2026, 12, 31)).strftime('%Y-%m-%d')
    status = random.choice(statuses)
    
    rush_order = 1 if 'Rush' in speed_estimate else 0
    is_warranty = random.choices([0, 1], weights=[0.9, 0.1])[0]
    deposit = random.choice([0.0, 20.0, 50.0])
    
    cursor.execute("""
        INSERT INTO Ticket (intake_date, problem, speed_estimate, device_name, passcode, status, rush_order, total_price, is_warranty, deposit, customer_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, 0.0, ?, ?, ?);
    """, (intake_date, problem, speed_estimate, device_name, password, status, rush_order, is_warranty, deposit, chosen_id))
    
    t_no = cursor.lastrowid
    ticket_total_price = 0.0
    
    # 25% of tickets get a second repair to push the 200 tickets to ~250 total repairs
    num_repairs_for_ticket = random.choices([1, 2], weights=[0.75, 0.25])[0]
    
    for _ in range(num_repairs_for_ticket):
        rep_task, min_p, max_p = random.choice(repair_types)
        repair_price = round(random.uniform(min_p, max_p), 2)
        
        if rush_order == 1: repair_price += 20.0
        if is_warranty == 1: repair_price = 0.0
            
        chosen_tech = random.choice(tech_ids)
        cursor.execute("""
            INSERT INTO Repair (repair_type, price, ticket_no, employee_id)
            VALUES (?, ?, ?, ?);
        """, (rep_task, repair_price, t_no, chosen_tech))
        r_id = cursor.lastrowid
        total_repairs_generated += 1
        
        ticket_total_price += repair_price
        
        # Most repairs need 1-3 parts to push our 250 repairs to ~400 total parts
        num_parts_for_repair = random.choices([1, 2, 3], weights=[0.5, 0.4, 0.1])[0]
        
        # Use random.sample to guarantee we don't pick the same part_id twice for one repair (Composite PK safety)
        sampled_part_ids = random.sample(part_pool, num_parts_for_repair)
        
        for chosen_part_id in sampled_part_ids:
            is_defective = random.choices([0, 1], weights=[0.94, 0.06])[0]
            cursor.execute("""
                INSERT INTO UsedPart (repair_id, part_id, quantity, is_defective)
                VALUES (?, ?, 1, ?);
            """, (r_id, chosen_part_id, is_defective))
            total_parts_generated += 1
            
    # Update Ticket Total Price with the accumulated repair costs
    cursor.execute("UPDATE Ticket SET total_price = ? WHERE ticket_no = ?;", (ticket_total_price, t_no))
    
    # Generate Payments for completed jobs
    if status in ['Done', 'Picked up'] and ticket_total_price > 0:
        cursor.execute("""
            INSERT INTO Payment (payment_amount, payment_method, ticket_no)
            VALUES (?, ?, ?);
        """, (ticket_total_price, random.choice(['check', 'card', 'cash']), t_no))
        
    # Generate random Contact Events
    if random.random() < 0.4:
        c_type = random.choice(['call', 'sms', 'email'])
        contact_address = fake.phone_number() if c_type != 'email' else fake.email()
        cursor.execute("""
            INSERT INTO ContactEvent (message, contact_type, contact_address, ticket_no)
            VALUES ('Status update regarding your device.', ?, ?, ?);
        """, (c_type, contact_address, t_no))

conn.commit()
print(f"Generated {num_tickets} Tickets, {total_repairs_generated} Repairs, and {total_parts_generated} Used Parts.")